...
class ApiService {
  ...
  static Future<List<Dog>> fetchDogs() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return data
            .where((dog) => dog is Map<String, dynamic>)
            .map((dog) => Dog.fromJson(dog as Map<String, dynamic>))
            .toList();
      } else {
  ...
  static Future<void> updateDog(Dog dog, XFile? imageFile) async {
    try {
      ...
      if (imageFile != null) {
        if (kIsWeb) {
          // Web: Use MultipartFile fromBytes for web
